...
class Encoder_default(ResNet):
    """
    FTWS: The encoder which take real pictures and return its corresponding latent code, through a typical ResNet50 arch
    without the final linear layer.
    In further development, this Network will be adopted to the prop of labeled imgs and automatically reform its dept
    -hs.
    The users should flatten the output like
    x = torch.flatten(x, 1)
    if they want features with shape [batch_size, feature_dim]
    """
    def __init__(self, dim_z, skip_init=False, E_lr=2e-4, E_init='ortho', widden_factor=2,
                 adam_eps=1e-8, E_B1=0.0, E_B2=0.999, E_mixed_precision=False, name=None, **kwargs):
        super(Encoder_default, self).__init__(Bottleneck, [3, 4, 6, 3], width_per_group=64*widden_factor)
        self.init = E_init
        if not skip_init:
            self.init_weights()
        # Set name used for save and load weights
...
class Encoder_out_layer(nn.Module):
    def __init__(self, dim_z, skip_init=False, E_lr=2e-4, E_init='ortho', widden_factor=2,
                 adam_eps=1e-8, E_B1=0.0, E_B2=0.999, E_mixed_precision=False, name=None, **kwargs):
...
def Encoder(arch='ResBlock_inv', **kwargs):
    encoder_dict = {"ResBlock_inv": Encoder_inv, "ResBlock_rand": Encoder_rd,
                    'default': Encoder_default, 'out': Encoder_out_layer}
